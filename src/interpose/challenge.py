...
_NOTE = (
    "Content digests of the policies as frozen, covering ONLY the file each "
    "policy class is defined in -- not the modules its behaviour depends on. "
    "RETRACTED 2026-08-31: an earlier version of this note claimed the commit "
    "adding a policy here predates the attacks that score it. The project's own "
    "git history contradicts that for confidential-egress, whose scenario and "
    "the R3 rewrite it forced landed in one commit. The ordering rule is a "
    "going-forward commitment that has not yet been honoured for any shipped "
    "scenario. See docs/research/RETRACTIONS.md."
)
